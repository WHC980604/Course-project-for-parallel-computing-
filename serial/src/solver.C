	...
	for(int nIters = 0; nIters <= maxIterations; nIters++)
	{
		...
		for(int i = 1; i < N - 1; i++)
		{
			for(int j = 1; j < N - 1; j++)
			{
				...
				pb.R[k] = ((pb.omega[k + N] - 2. * pb.omega[k] + pb.omega[k - N]) / ((dh) * (dh)) + (pb.omega[k + 1] - 2. * pb.omega[k] + pb.omega[k - 1]) / ((dh) * (dh))) / Re 
					- (pb.omega[k + N] * pb.u[k + N] - pb.omega[k - N] * pb.u[k - N]) / (2. * dh) - (pb.omega[k + 1] * pb.v[k + 1] - pb.omega[k - 1] * pb.v[k - 1]) / (2. * dh);
		...
		if(errR < residualTolerance && errS < residualTolerance && nIters > 5) break;
	}
